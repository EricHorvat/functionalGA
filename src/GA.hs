...
nextGen selectMethod k crossMethod mutateMethod replaceMethod selectMethod4replace fitness (population, seed) =
  (replaceMethod population mutated s4 selectMethod4replace fitness, sEnd) where
    mutated = mutate mutateMethod s3 crossed
    crossed = cross crossMethod s2 selected
